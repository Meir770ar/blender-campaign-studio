...
| Video fallback | Existing Genspark website account | See [genspark-fallback.md](genspark-fallback.md). Reuse the exact failed shot and first frame; `browser-check`, single-attempt `execute`, `record-browser`, then verified MP4 `register`. Native browser controls use the website's confirmed credit balance. |
